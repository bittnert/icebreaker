...
static void get_vpi_handles()
{
	...
	if (!vh)
		vl_fatal(__FILE__, __LINE__, "sim_main", "PC handle not found");

	reg_mem = vpi_handle_by_name((PLI_BYTE8 *)"TOP.cpu.register_file.reg_a.mem", NULL);
	if (!reg_mem)
		vl_fatal(__FILE__, __LINE__, "sim_main", "Register memory handle not found");

	mem_wr = vpi_handle_by_name((PLI_BYTE8 *)"TOP.cpu.mem_wr", NULL);
	if (!mem_wr)
		vl_fatal(__FILE__, __LINE__, "sim_main", "Memory write handle not found");
	...
	if (!mem_en)
		vl_fatal(__FILE__, __LINE__, "sim_main", "Memory enable handle not found");
	...
	if (!state)
		vl_fatal(__FILE__, __LINE__, "sim_main", "State handle not found");
	...
	if (!tx_fill_level)
		vl_fatal(__FILE__, __LINE__, "sim_main", "tx fill level handle not found");

	rx_fill_level = vpi_handle_by_name((PLI_BYTE8 *)"TOP.cpu.load_store_unit.uart.uart.rx_fifo_fill_lvl", NULL);
	if (!rx_fill_level)
		vl_fatal(__FILE__, __LINE__, "sim_main", "rx fill level handle not found");
}
...
bool process_map_file(char *filename, uint32_t *sig_start, uint32_t *sig_end, uint32_t *code_end, uint32_t *ram_base, uint32_t *tohost)
{
	std::ifstream file(filename);
	if (!file.is_open())
	{
	...
	std::regex tohost_regex("^\\s*0x([0-9A-Fa-f]{8})\\s*tohost");
	std::string line;
	std::smatch matches;
	...
	while (std::getline(file, line))
	{
		...
		if (std::regex_search(line, matches, tohost_regex))
		{
			std::string hex_str = matches[1].str();
			*tohost = std::stoul(hex_str.c_str(), nullptr, 16);
			found_tohost = true;
			printf("Found tohost: 0x%08x %s\n", tohost, hex_str.c_str());
		}
	}

	file.close();

	if (found_sig_end && found_code_end && found_sig_start && found_ram_base && found_tohost)
	{
		return true;
	}
...
{
	uint32_t sig_start, sig_end, code_end, ram_base, tohost, tohost_offset;
	...
	if (!process_map_file(argv[argc - 1], &sig_start, &sig_end, &code_end, &ram_base, &tohost))
	{
		printf("Error: Failed to parse map file %s\n", argv[argc - 1]);
		return 1;
	}

	tohost_offset = tohost - ram_base;
	...
	vpiHandle tohost_handle = vpi_handle_by_index(mem_hdl, tohost_offset / 4);
	if (!tohost_handle)
	{
		printf("Error: Failed to get ToHost handle\n");
		return 1;
	}

	t_vpi_value state_val;
	t_vpi_value tohost_val;
	tohost_val.format = vpiIntVal;
	tohost_val.value.integer = 0;
	state_val.format = vpiIntVal;
	...
	vpi_get_value(tohost_handle, &tohost_val);
	while ((tohost_val.value.integer == 0) && !(pc.value.integer & 3 != 0))
	{
		...
		{
#if 0
			if (last_pc == pc.value.integer) {

				printf("Stall detected at PC: 0x%08x\n", pc.value.integer);
				log_data("Stall detected at PC: 0x" + std::to_string(pc.value.integer));
				break;
			}
#endif
		...
		log_data(std::format("PC: 0x{:08x}", static_cast<uint32_t>(pc.value.integer)));
		if (pc.value.integer == 0x208)
		{
